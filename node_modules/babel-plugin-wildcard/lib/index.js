...
var name=state.file.opts.filename;var files=[];var isDirs=[];var dir=_path3.default.join(_path3.default.dirname(name),src);// path of the target dir.
...
var _path$parse=_path3.default.parse(r[i]),_name=_path$parse.name,ext=_path$parse.ext;if(exts.indexOf(ext.substring(1))>-1&&filenameRegex.test(_name)){files.push(r[i]);isDirs.push(!ext)}}}catch(e){console.warn('Wildcard for '+name+' points at '+src+' which is not a directory.');return}// This is quite a mess but it essentially formats the file
// extension, and adds it to the object
for(var i=0;i<files.length;i++){// name of temp. variable to store import before moved
// to object
var id=path.scope.generateUidIdentifier('wcImport');var file=files[i];var isDir=isDirs[i];// Strip extension
